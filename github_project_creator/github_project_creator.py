...
DEFAULT_GCP_PROJECT_NAME = "my-gcp-project"
...
STANDARD_TEAM_ATTRIBUTES = {
    ...
    "permission": "push",
    "description": 'Standard team for ' + DEFAULT_GCP_PROJECT_ID + ' project',
    "privacy": "closed"
}
PRIV_TEAM_ATTRIBUTES = {
    "name": DEFAULT_GCP_PROJECT_NAME + '-priv',
    "permission": "push",
    "description": 'Privileged team for ' + DEFAULT_GCP_PROJECT_ID + ' project',
    "privacy": "secret"
}
PROJECT_DATA_DIR = \
    MODULE_ROOT_DIR + 'resources/project_data/' + DEFAULT_GCP_PROJECT_ID
PROTECTED_BRANCH = {
    "enforce_admins": True,
...
def arg_parser():
    ...
    parser.add_argument('-o', '--org',
                        help="The Github organisation we're operating on",
                        default=DEFAULT_GITHUB_ORG)
    parser.add_argument('-p', '--project-name',
                        help="Name of project we're creating a repository for",
                        default=DEFAULT_GCP_PROJECT_NAME)
...
def get_repo(org, name=DEFAULT_GCP_PROJECT_ID):
    repo = None
    try:
...
def create_repo(org, name=DEFAULT_GCP_PROJECT_ID):
    """
...
def main():
    ...
    environment = settings.project_name.upper().split('-').pop()
    try:
    ...
    existing_repo = get_repo(org, settings.project_name)

    if existing_repo and not settings.force:
        print("Repository " + settings.project_name
              + " already exists. Use --force to reconfigure")
        exit(1)
        ...
        repo = create_repo(org, name=settings.project_name)
